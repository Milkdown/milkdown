  ...
  waitForInstance(config.args, 5000)
    .then((crepe) => {
      ...
      startBtn.addEventListener('click', () => {
        ...
        // Poll for streaming end. Only enable diff controls if diff
        // decorations are actually present (streaming completed
        // normally with diffReview: true, not aborted / errored).
