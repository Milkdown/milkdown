...
/// Simulated AI provider that streams a markdown string in small chunks.
...
export function setupAIDemo(config: setupConfig) {
  ...
  textarea.placeholder = 'Enter markdown content for AI to stream...'
  ...
  waitForInstance(config.args, 5000)
    .then((crepe) => {
      ...
      startBtn.addEventListener('click', () => {
        setStreaming(true)
        // runAICmd takes the instruction (the textarea content) and
        // hands it to the provider configured at Crepe construction.
        // The provider streams it back char by char, then
        // endStreamingCmd fires with diffReview: true.
        crepe.editor.action(
          callCommand(runAICmd.key, { instruction: textarea.value })
        )
        // The streaming → diff handoff is automatic; we just need to
        // re-enable the diff buttons when streaming ends. A simple
        // poll check is fine for a demo.
