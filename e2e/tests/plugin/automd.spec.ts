...
test.describe('keep mark symbol', () => {
  ...
  test('not a bold', async ({ page }) => {
    ...
    await expect(page.locator('.editor p')).toHaveText('The lunatic is o__"n the grass__');
    expect(await getMarkdown(page)).toBe('The lunatic is o\\_\\_"n the grass\\_\\_\n');
  });
  ...
  test('escape _', async ({ page }) => {
    ...
    await expect(page.locator('.editor')).toContainText('_\u200B_on the grass__');
  });
  ...
  test('not an italic', async ({ page }) => {
    ...
    await expect(page.locator('.editor p')).toHaveText('The lunatic is o*"n the grass*');
  ...
  test('inline code with * and _', async ({ page }) => {
    ...
    expect(await getMarkdown(page)).toBe('The lunatic is `_**on the grass**_`\n');
  });
});
...
test.describe('with inline math', () => {
  test('basic input', async ({ page }) => {
    ...
    await expect(page.locator('span[data-type="math_inline"]')).toHaveAttribute('data-value', 'E = MC^2');
  ...
  test('input with _', async ({ page }) => {
    ...
    await expect(page.locator('span[data-type="math_inline"]')).toHaveAttribute('data-value', 'a');
