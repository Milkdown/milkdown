...
test('code block', async ({ page }) => {
  ...
  await expect(page.locator('pre:first-child')).not.toHaveAttribute('data-language');
  await expect(page.locator('pre:last-child')).toHaveAttribute('data-language', 'javascript');
});
