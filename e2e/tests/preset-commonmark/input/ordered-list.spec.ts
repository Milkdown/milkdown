...
test('ordered list', async ({ page }) => {
  ...
  await expect(editor.locator('ol ol li:last-child')).toHaveText('Sub list item 2')
  markdown = await getMarkdown(page);
  expect(markdown).toBe('1. First item\n2. Second item\n\n   1. Sub list item 1\n   2. Sub list item 2\n');
  ...
  await expect(editor.locator('ol:first-child>li:last-child')).toHaveText('Third item')
  markdown = await getMarkdown(page);
  expect(markdown).toBe('1. First item\n2. Second item\n\n   1. Sub list item 1\n   2. Sub list item 2\n3. Third item\n');
});
