...
test('bullet list', async ({ page }) => {
  ...
  await expect(editor.locator('ul ul li:last-child')).toHaveText('Sub list item 2')
  markdown = await getMarkdown(page);
  expect(markdown).toBe('* List item 1\n* List item 2\n\n  * Sub list item 1\n  * Sub list item 2\n');
  ...
  await expect(editor.locator('ul:first-child>li:last-child')).toHaveText('List item 3')
  markdown = await getMarkdown(page);
  expect(markdown).toBe('* List item 1\n* List item 2\n\n  * Sub list item 1\n  * Sub list item 2\n* List item 3\n');
});
