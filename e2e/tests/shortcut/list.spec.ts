...
test('ordered list', async ({ page }) => {
  ...
  await expect(editor.locator('ol>li').first()).toHaveText('The lunatic is on the grass')
...
test('bullet list', async ({ page }) => {
  ...
  await expect(editor.locator('ul>li').first()).toHaveText('The lunatic is on the grass')
  await page.keyboard.press('Enter')
  await page.keyboard.type('The lunatic is in the hall')
